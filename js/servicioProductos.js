...
miAplicacion.service('ServicioProductos', function ($http) {
    ...
    this.traerTodo = function () {
      return $http.get('http://localhost:8080/labor/ws/productos').then(function(respuesta) {    
         return respuesta.data.listado;    
      });
    }

    this.Agregar = function(dato){
      return $http.post('http://localhost:8080/labor/ws/producto/'+dato).then(function(respuesta){
        return respuesta.data;
      })
    }

    this.Modificar = function(dato){
      return $http.put('http://localhost:8080/labor/ws/producto/'+dato).then(function(respuesta){
